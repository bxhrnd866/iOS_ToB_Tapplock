...
class RegisterUserImageUploader: NSObject, Uploader {
    ...
    func saveToApi(url: String) {
        _ = provider.rx.request(ApiService.EditUserConfiguration(mai: (ConfigurationModel.instance.user.value?.mail)!, deviceToken: nil, firstName: nil, lastName: url, imageURL: nil, push: nil, showBattery: nil))
                .mapObject(APIResponse<UserModel>.self)
                .subscribe(onSuccess: { response in
                    if let user = response.data {
                        self.rx_sucess.value = true
                        
                        ConfigurationModel.instance.user.value = user
